...
lis.grabber = function(directory = "current"){
     ...
     return(lis.subset)
     # return(filtered.lis)
     # lis.pcr.join = left_join(pcrexport, lis.subset, by=c("test.order"="Test.order..", "Run.ID"))
     # 
     # final.filter <- lis.pcr.join %>%
     #      # select(Sample.Name, Interpretation.6:Interpretation.10) %>%
...
categorizer = function(df, assay="paraflu", inputfile){
     ...
     if(missing(inputfile)){
          ...
          for(i in 1:length(inputfile$term)){
               df$sample.type[grepl(inputfile$term[i], df$Sample.Name, ignore.case=TRUE)] = inputfile$type[i]
          }
          return(df)
     }
}
