...
# remove_outlier(x=c(1.2,2.2,3.2,1.2,2.2,3.2,9.2, 9))
...
#logic: if any are retries available then recalculate
if(any(retry.lv)){
  ...
  print("Spec passes and no outliers were filtered.")
}
...
uncategorized.observations <- join.wlid.limits.ic.hiv %>% filter(SampleType == "Uncategorized")
if(dim(uncategorized.observations)[1] == 0){
  print("Every sample has been categorized.")
} else{
  print(paste0("Double check Work List ID, some samples have been uncategorized and excluded from analysis. Number of uncategorized samples:",
        dim(uncategorized.observations)[1],". See uncategorized sample names below:"))
  print(join.wlid.limits.ic.hiv %>% filter(SampleType == "Uncategorized") %>% select(Sample.Name) %>% unique)
}
...
```{r}
join.wlid.limits.ic.hiv %>%
  filter(FALSE == GIC.pass |
           FALSE == HIVSpecPass |
           (!is.na(recovery.spec.pass) & !recovery.spec.pass)) %>%
  select(Run.ID, Sample.Name, SampleType, Test.order, Instrument.Flags, Interpretation.2_min, Interpretation.2, Interpretation.2_max, GIC.TT, GIC.TT_max,
         GIC.pass, HIVSpecPass, Logdiff, recovery.spec.pass) %>%
...
# library(plotly)
# a <- join.wlid.limits.ic.hiv %>%
#   mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
#   ggplot(aes(x=SampleTestOrder, y=POL.TT)) + geom_point(aes(color=SampleType)) +
#   facet_wrap(~Serial.Number)
# ggplotly(a)
...
Instrument-to-Instrument Plots
=======================================================================
Row {.tabset .tabset-fade}
-----------------------------------------------------------------------
###GIC TT
...
a <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=GIC.TT)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())

ggplotly(a, width=1000, height=500)
...
```{r}
b <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=POL.TT)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())

ggplotly(b, width=1000, height=500)
...
```{r}
c <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=LTR.TT)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())

ggplotly(c, width=1000, height=500)
...
```{r}
require(plotly)
d <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=GIC.RR)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())
...
```{r}
e <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=POL.RR)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())

ggplotly(e, width=1000, height=500)
...
```{r}
f <- join.wlid.limits.ic.hiv %>%
  mutate(SampleTestOrder = paste(Sample.Name, Test.order)) %>%
  ggplot(aes(x=SampleTestOrder, y=LTR.RR)) + geom_point(aes(color=SampleType)) +
  facet_wrap(~Serial.Number, nrow=1) +
  theme(axis.text.x=element_blank())

ggplotly(f, width=1000, height=500)
